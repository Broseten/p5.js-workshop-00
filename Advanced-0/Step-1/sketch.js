...
function draw() {
  ...
  let strokeW = map(mouseX, 0, width, 10, 100);
  
  for (let i = 0; i < paths.length; i++) {
    paths[i].attribute("stroke-width", strokeW);
  }
  paths[0].attribute("fill", random(colors));
  paths[1].attribute("stroke", random(colors));
  
  // can add different svg filters or patterns if they are in the svg
  // paths[1].attribute('filter', 'url(#displacementFilter)');
}
