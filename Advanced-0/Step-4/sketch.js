...
function preload() {
  ...
  for (let i = 0; i < svgCount; i++) {
    loaded_svgs.push(loadSVG("svgs/" + i + ".svg"));
  }
}
...
function draw() {
  ...
  let c0 = random(colors);

  let bgHue = hue(c0);
  let bgSat = saturation(c0);
  let bgBri = brightness(c0);
  let bgColor = color(bgHue, bgSat * 0.8, bgBri * 0.4);
  background(bgColor);
  ...
  let cols = 4;
  let rows = cols;
  let svg_size = width / cols;
  let i = 0;
  for (let row = 0; row < rows; row++) {
    for (let col = 0; col < cols; col++) {
      let svg = loaded_svgs[i % loaded_svgs.length];
      let svg_id = "gen-svg-" + i;
      svg.elt.setAttribute("id", svg_id);

      image(svg, row * svg_size, col * svg_size, svg_size, svg_size);

      let svgElement = document.getElementById(svg_id);
      let paths2 = svgElement.getElementsByTagName("path");
      colors = shuffle(colors);
      for (let i = 0; i < paths2.length; i++) {
        let colorIndex = i % colors.length;
        paths2[i].setAttribute("stroke-width", "7%");
        paths2[i].setAttribute("stroke", colors[colorIndex + 1]);
        if (paths2[i].getAttribute("fill") !== "none") {
          paths2[i].setAttribute("fill", colors[colorIndex]);
        }
      }
      i++;
    }
