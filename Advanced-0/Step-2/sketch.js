...
function preload() {
  svg = loadSVG("test-affinity-opt.svg");
}
...
function draw() {
  ...
  let colors = shuffle(random(palette));
  let c0 = colors.pop();
  let c1 = colors.pop();
  let c2 = colors.pop();
  let c3 = colors.pop();
  ...
  background(c0);
  let strokeW = map(mouseX, 0, width, 1, 15) + "%";
  
  
  //////// Option 1 ///////
  // does not allow you to select a specific SVG, but instead work with a whole SVG
  
  svg.elt.setAttribute("id", "svg0");
  image(svg, 0, height/4, 200, 200);
  // cannot specify which SVG we are reffering to -- instead uses whole canvas
  let paths = querySVG("path");
  for (let i = 0; i < paths.length; i++) {
    paths[i].attribute("stroke-width", strokeW);
    paths[i].attribute("stroke", c1);
    paths[i].attribute("fill", c2);
  }

  //////// Option 2 ///////
  // better, since you can select a specific SVG to work with
  
  svg.elt.setAttribute("id", "svg1");
  image(svg, 200, height/4, 200, 200);
  let svgElement = document.getElementById("svg1");
  let paths2 = svgElement.getElementsByTagName("path");

  for (let i = 0; i < paths2.length; i++) {
    paths2[i].setAttribute("stroke-width", strokeW);
    paths2[i].setAttribute("stroke", c3);
    paths2[i].setAttribute("fill", c4);
  }
  
  
  // btw can add different svg filters or patterns if they are in the svg: paths[1].attribute('filter', 'url(#displacementFilter)');
}
